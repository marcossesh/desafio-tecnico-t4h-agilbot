...
overlayfs, enquanto `app/data` é um bind mount, ou seja, outro filesystem. `os.replace` entre
...
readonly no bash**, então a variável se chama `APP_UID`, porque `UID=$(id -u) docker compose ...`
...
já que escrever handlers novos é o ponto de extensão mais provável do sistema.
...
| --- | --- | --- | --- |
| `gemini-2.5-flash-lite` | ✗ 404 (aposentado para chaves novas) | n/a | n/a |
...
| `gemini-2.0-flash`, `2.0-flash-lite` | ✗ 429 | n/a | n/a |
...
porque `gemini-flash-lite-latest` é um alias móvel.
